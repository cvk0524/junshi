---
name: junshi-qa-only
description: "探马 — QA 报告模式：只发现和记录，不修复"
---
...
agent-browser screenshot --output "$REPORT_DIR/screenshots/initial.png"
```
...
**For SPAs:** Use `snapshot` to find nav elements (buttons, menu items) instead of relying on links.
...
agent-browser snapshot
agent-browser screenshot --output "$REPORT_DIR/screenshots/page-name.png"
```
...
```bash
agent-browser screenshot --output "$REPORT_DIR/screenshots/issue-001-step-1.png"
agent-browser click @e5
agent-browser screenshot --output "$REPORT_DIR/screenshots/issue-001-result.png"
```
...
- Use `snapshot` for navigation — links command misses client-side routes
