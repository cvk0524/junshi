---
name: junshi-design-review
description: "御史 — 设计审计+修复：10 类 80+ 检查项，AI 味检测"
---
...
2. Take a full-page desktop screenshot: `agent-browser screenshot --output "$REPORT_DIR/screenshots/first-impression.png"`
...
agent-browser execute "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).map(e => getComputedStyle(e).fontFamily))])"

# Color palette in use
agent-browser execute "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).flatMap(e => [getComputedStyle(e).color, getComputedStyle(e).backgroundColor]).filter(c => c !== 'rgba(0, 0, 0, 0)'))])"

# Heading hierarchy
agent-browser execute "JSON.stringify([...document.querySelectorAll('h1,h2,h3,h4,h5,h6')].map(h => ({tag:h.tagName, text:h.textContent.trim().slice(0,50), size:getComputedStyle(h).fontSize, weight:getComputedStyle(h).fontWeight})))"

# Touch target audit (find undersized interactive elements)
agent-browser execute "JSON.stringify([...document.querySelectorAll('a,button,input,[role=button]')].filter(e => {const r=e.getBoundingClientRect(); return r.width>0 && (r.width<44||r.height<44)}).map(e => ({tag:e.tagName, text:(e.textContent||'').trim().slice(0,30), w:Math.round(e.getBoundingClientRect().width), h:Math.round(e.getBoundingClientRect().height)})).slice(0,20))"
```
...
agent-browser screenshot --output "$REPORT_DIR/screenshots/{page}-annotated.png"
```
...
- `prefers-reduced-motion` respected (check: `agent-browser execute "matchMedia('(prefers-reduced-motion: reduce)').matches"`)
...
10. **Show screenshots to the user.** After every screenshot command, use the Read tool on the output file(s) so the user can see them inline. This is critical — without it, screenshots are invisible to the user.
...
agent-browser screenshot --output "$REPORT_DIR/screenshots/finding-NNN-after.png"
```
