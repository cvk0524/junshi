---
name: junshi-design-consultation
description: "工匠 — 设计系统咨询：创建 DESIGN.md 与预览页面"
---
...
If the codebase is empty and purpose is unclear, say: *"I don't have a clear picture of what you're building yet. Want to brainstorm first? Once we know the product direction, we can set up the design system."*

**Find the agent-browser (optional — enables visual competitive research):**

```bash
which agent-browser >/dev/null 2>&1 && echo "BROWSER_READY" || echo "NO_BROWSER"
```
...
**Step 2: Visual research via agent-browser (if available)**

If agent-browser is available, visit the top 3-5 sites in the space and capture visual evidence:

```bash
agent-browser navigate "https://example-site.com"
agent-browser screenshot --output "/tmp/design-research-site-name.png"
...
If agent-browser is not available, rely on WebSearch results and your built-in design knowledge — this is fine.
...
- agent-browser available → screenshots + snapshots + WebSearch (richest research)
- agent-browser unavailable → WebSearch only (still good)
...
**Update project instructions** (CLAUDE.md or equivalent) — append this section:
...
- A) Ship it — write DESIGN.md and update project instructions
